...
async function seedPrerequisites(adminPool, fixture) {
  ...
  for (const event of ledger) {
    ...
    await adminPool.query(`
      ...
      ) values($1,$2,$3,$4,$5::timestamptz,$6::text,$7,$8,$9::jsonb,$10,$11,$12,$13)
    `, [event.tenantId, event.jobId, event.sequence, event.eventType, event.occurredAt, event.occurredAt, event.previousEventSha256, event.eventSha256, JSON.stringify(event.payload), output?.manifestSha256 ?? null, output?.manifestByteLength ?? null, output?.modelSha256 ?? null, output?.modelByteLength ?? null]);
  }
  await adminPool.query(`
