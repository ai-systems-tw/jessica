...
/** Canonical signed JSC-0215 payload reconstructed from one persisted terminal row. */
export function nonProxyHumanQaSignedPayloadFromRecord(row: NonProxyHumanQaRecordRow): Omit<NonProxyHumanQaDecisionAttestation, "signatureBase64"> {
  return {
    schemaVersion: 1, type: "non-proxy-human-qa-decision-attestation", algorithm: "ES256", scope: NON_PROXY_HUMAN_QA_SCOPE,
    authorityId: row.reviewerAuthorityId, keyId: row.reviewerKeyId, publicKeyFingerprintSha256: row.reviewerPublicKeyFingerprintSha256,
    reviewerId: row.reviewerId, tenantId: row.tenantId, frameModelId: row.frameModelId, frameVariantId: row.frameVariantId,
    candidateId: row.candidateAssetVersionId, candidateVersion: row.candidateVersion, jobId: row.generationJobId,
    canonicalInputSha256: row.canonicalInputSha256, reviewHeadEventSha256: row.reviewHeadEventSha256, generatorInputSha256: row.generatorInputSha256,
    output: row.output, sourceAssetSha256s: row.sourceAssetSha256s, measurementSetSha256: row.measurementSetSha256, specimenId: row.specimenId,
    composition: row.composition, rightsScope: row.rightsScope, decision: row.decision, issueCategories: row.issueCategories, notes: row.notes,
    approvedQualityEnvelope: row.approvedQualityEnvelope, reviewedAt: row.reviewedAt, issuedAt: row.issuedAt, expiresAt: row.expiresAt,
  };
}
...
export async function inspectNonProxyQaPersistencePlanIntegrity(value: unknown): Promise<NonProxyQaPersistencePlan> {
  ...
  const signedPayload = nonProxyHumanQaSignedPayloadFromRecord(copy.reviewRecord as unknown as NonProxyHumanQaRecordRow);
