...
const ASSET_KEYS = ["id","persistence_row_sha256","tenant_id","frame_model_id","frame_variant_id","version","generation_job_id","quality","generation_method","model_url","manifest_url","manifest_sha256","manifest_byte_length","model_sha256","model_byte_length","source_set_sha256","attachment_matrix","quality_envelope","status","fixture_status","review_status","admission","promotable","rights_scope","recommended_for_live","publication_eligible","non_proxy_internal_review"] as const;
function assetProjection(row: Record<string, unknown>): NonProxyAssetVersionRow { if (!bool(row.non_proxy_internal_review)) fail(); return { id: id(row.id), rowSha256: hash(row.persistence_row_sha256), tenantId: id(row.tenant_id), frameModelId: id(row.frame_model_id), frameVariantId: id(row.frame_variant_id), version: integer(row.version), generationJobId: id(row.generation_job_id), quality: text(row.quality) as "standard"|"premium", generationMethod: text(row.generation_method) as "standard-auto"|"manual"|"external", modelUrl: text(row.model_url), manifestUrl: text(row.manifest_url), manifestSha256: hash(row.manifest_sha256), manifestByteLength: integer(row.manifest_byte_length), modelSha256: hash(row.model_sha256), modelByteLength: integer(row.model_byte_length), sourceSetSha256: hash(row.source_set_sha256), attachmentMatrix: json<number[]>(row.attachment_matrix), qualityEnvelope: json<NonProxyApprovedQualityEnvelope>(row.quality_envelope), status: text(row.status) as "approved", fixtureStatus: text(row.fixture_status) as "unverified", reviewStatus: text(row.review_status) as "approved", admission: text(row.admission) as "internal-review-only", promotable: bool(row.promotable) as false, rightsScope: text(row.rights_scope) as "internal-review-only", recommendedForLive: bool(row.recommended_for_live) as false, publicationEligible: bool(row.publication_eligible) as false }; }
const BINDING_KEYS = ["id","row_sha256","tenant_id","review_record_id","asset_version_id","frame_model_id","frame_variant_id","generation_job_id","source_set_sha256","quality_envelope","decision_payload_sha256","effective_valid_until","asset_version_row_sha256","rights_scope","recommended_for_live","publication_eligible"] as const;
function bindingProjection(row: Record<string, unknown>): NonProxyAssetVersionBindingRow { return { id: id(row.id), rowSha256: hash(row.row_sha256), tenantId: id(row.tenant_id), reviewRecordId: id(row.review_record_id), assetVersionId: id(row.asset_version_id), frameModelId: id(row.frame_model_id), frameVariantId: id(row.frame_variant_id), generationJobId: id(row.generation_job_id), sourceSetSha256: hash(row.source_set_sha256), qualityEnvelope: json<NonProxyApprovedQualityEnvelope>(row.quality_envelope), decisionPayloadSha256: hash(row.decision_payload_sha256), effectiveValidUntil: timestamp(row.effective_valid_until), assetVersionRowSha256: hash(row.asset_version_row_sha256), rightsScope: text(row.rights_scope) as "internal-review-only", recommendedForLive: bool(row.recommended_for_live) as false, publicationEligible: bool(row.publication_eligible) as false }; }
const SOURCE_KEYS = ["persistence_source_row_id","persistence_source_row_sha256","tenant_id","asset_version_id","frame_model_id","frame_variant_id","source_asset_id","source_sha256"] as const;
function sourceProjection(row: Record<string, unknown>): NonProxyAssetVersionSourceRow { return { id: id(row.persistence_source_row_id), rowSha256: hash(row.persistence_source_row_sha256), tenantId: id(row.tenant_id), assetVersionId: id(row.asset_version_id), frameModelId: id(row.frame_model_id), frameVariantId: id(row.frame_variant_id), sourceAssetId: id(row.source_asset_id), sourceSha256: hash(row.source_sha256) }; }
